...
exports.handler = async (event, _context) => {
  ...
  const videoName = event["queryStringParameters"]["video"];
  const embedding = await getEmbedding(searchText);
  const results = await querySingleStore(connection, embedding, videoName);
...
async function querySingleStore(connection, embedding, videoName) {
  const [videos] = await connection.execute(
    "SELECT video, dot_product(vector, JSON_ARRAY_PACK(?)) AS score, timestamp FROM videos WHERE score > 0.7 AND video = ? ORDER BY CAST(timestamp AS UNSIGNED) ASC",
    [JSON.stringify(embedding), videoName]
  );

  const [transcriptions] = await connection.execute(
    "SELECT video, dot_product(vector, JSON_ARRAY_PACK(?)) AS score, timestamp FROM transcriptions WHERE score > 0.4 AND video = ? ORDER BY CAST(timestamp AS UNSIGNED) ASC",
    [JSON.stringify(embedding), videoName]
  );

  return { videos, transcriptions };
}
